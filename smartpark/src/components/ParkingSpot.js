...
const ParkingSpot = ({ spot, setStart, setEnd }) => {
  ...
  if (!spot) {
    return null; // or handle the case where spot is not defined
  }
      ...
      <Card variant="outlined" sx={{ minWidth: 275, marginBottom: 2 }} onClick={handleCardClick} style={{ cursor: 'pointer' }}>
        <CardContent>
          <Typography variant="h5" component="div">
            {spot.name} {/* Ensure spot.name is defined */}
          </Typography>
          <Typography sx={{ fontSize: 14 }} color="text.secondary" gutterBottom>
            Availability: {spot.availability} {/* Ensure spot.availability is defined */}
          </Typography>
          <Typography variant="body2">
            Price: ${spot.price}/hour {/* Ensure spot.price is defined */}
          </Typography>
...
export default ParkingSpot; // Exporting the component itself, not parkingSpots
